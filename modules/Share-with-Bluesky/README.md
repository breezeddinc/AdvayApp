...
iOS Share Extension for the ADVAY Social app that enables users to share content from other apps directly to ADVAY.

## Overview

This module implements an iOS Share Extension (Action Extension) that appears in the system share sheet when users tap the share button in other iOS apps. It allows sharing text, URLs, images, and videos to create a new ADVAY post.
...
3. Media files are copied to a shared App Group container (`group.com.advay.app`) for access by the main app
...
6. The main ADVAY app is opened with the deep link
...
- `Share-with-ADVAY.entitlements` - App group entitlements for shared file access
...
The extension uses the `group.com.advay.app` App Group identifier to share files with the main app. This is configured in:

- `Share-with-ADVAY.entitlements`
...
2. Navigate to the Share-with-ADVAY target
