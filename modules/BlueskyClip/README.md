...
An iOS App Clip implementation for ADVAY starter packs. App Clips are lightweight app experiences that allow users to preview and join ADVAY through starter packs without installing the full app.

## What It Does

BlueskyClip provides a minimal, on-demand iOS app experience for viewing and joining ADVAY starter packs. When a user encounters a starter pack link (e.g., `bsky.app/start/...` or `go.bsky.app/...`), iOS can present the App Clip instead of requiring a full app install. The App Clip:
...
**Data Sharing**:
- Uses UserDefaults suite `group.com.advay.app` (App Group)
...
- `com.apple.security.application-groups`: `group.com.advay.app`
- `com.apple.developer.associated-appclip-app-identifiers`: Links to the App Clip bundle ID

**App Clip** (`BlueskyClip.entitlements`):
- `com.apple.security.application-groups`: `group.com.advay.app` (for data sharing)
