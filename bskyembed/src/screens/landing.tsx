...
function LandingPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      try {
        ...
        if (uri) {
          if (uri.startsWith('at://')) {
            ...
            try {
              ...
              console.log(err)
              throw new Error('Invalid ADVAY URL')
            }
          }
        }
        ...
        console.error(err)
        setError(err instanceof Error ? err.message : 'Invalid ADVAY URL')
      } finally {
        setLoading(false)
      }
    })()
  ...
  return (
    <main className="w-full min-h-dvh flex flex-col items-center gap-8 py-14 px-4 md:pt-32 dark:text-slate-200">
      ...
      <h1 className="text-4xl font-bold text-center">Embed a ADVAY Post</h1>
