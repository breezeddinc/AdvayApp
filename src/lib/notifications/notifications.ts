...
/**
 * @private
 * Registers the device's push notification token with the ADVAY server.
 */
...
/**
 * Hook to register the device's push notification token with the ADVAY. If
...
/**
 * Hook to get the device push token and register it with the ADVAY server.
...
/**
 * Hook to register the device's push notification token with the ADVAY
...
export function useNotificationsRegistration() {
  ...
  useEffect(() => {
    ...
    /**
     * Register the push token with the ADVAY server, whenever it changes.
  ...
  return async (
    ...
    const res = await Notifications.requestPermissionsAsync({
      ios: {
         ...
         * screen for ADVAY. When tapped, iOS calls back into the app, which
